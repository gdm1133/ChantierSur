...
function renderAudit(doc, data, refDoc, currentDate) {
    ...
    console.log("PDF rendu : " + rawLines.length + " lignes apr�s construction des donn�es du PDF.");
    if (typeof rawLines === 'string') {
        try { rawLines = JSON.parse(rawLines); } catch (e) { rawLines = []; }
    }

    if (!Array.isArray(rawLines) || rawLines.length === 0) {
        throw new Error("Aucune ligne validée — retournez à l'écran de validation");
    }
    ...
    drawTable(
        ...
        [
            ...
            ['TVA applicable', tvaApplicable === 'oui' ? '18%' : 'Non appliquée (0%)', tvaApplicable === 'oui' ? `TVA calculée : ${fmt(tvaCalculee)} FCFA` : 'Vérifier si l\'entreprise bénéficie d\'une exonération légale.'],
    ...
    const analysePrixBody = computedLines.map(l => {
        ...
        return [l.des, `PU indiqué : ${fmt(l.pu)} FCFA/${l.u}`, 'À valider par un technicien sur site.'];
    });

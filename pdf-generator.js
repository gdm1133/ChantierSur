...
(function() {
  ...
  const REFERENCES_JURIDIQUES = {
    ...
    garantieDecennale: "Article 741 du Code Civil (Code Civil)",
    retenueGarantie: "Article 742 du Code Civil (Code Civil)",
    receptionTravaux: "Article 740 du Code Civil (Code Civil)",
    penalitesRetard: "Article 98 du Code Civil (Code Civil)"
  };
  ...
  function validerReferencesJuridiques(references) {
    ...
    for (let art of articlesInvalides) {
      if (JSON.stringify(references).includes(art)) {
        throw new Error(`Référence juridique erronée : ${art}. Le Code Civil traite du louage d'ouvrage aux articles 739 à 745.`);
      }
    }
    return true;
  }
  ...
  const NORMES_PAR_LOT = {
    grosOeuvre: "Eurocode 2 • DTU 13.12 (Fondations) • DTU 20.1 (Maçonneries)",
  ...
  function renderEsquisse(doc, data, refDoc, currentDate) {
    ...
    // Descente de charges Eurocode 2
    ...
    // =========================================================================
    doc.addPage();
    drawUnifiedHeader(doc, "Rapport d'Esquisse & Faisabilité Technique", "Partie II : Descente de Charges (Eurocode 2) & Dimensionnement des Fondations", refDoc, currentDate, clientName, clientPhone, lotNumber, 'esquisse');

    currentY = 52;
    drawSectionTitle(doc, currentY, "III. DESCENTE DE CHARGES THÉORIQUE SUR LE POTEAU LE PLUS CHARGÉ (Eurocode 2)");
    ...
    doc.autoTable(createTableOptions(
      currentY + TITLE_AFTER_GAP_MM,
      [['Paramètre de Descente de Charges', 'Valeur Calculée', 'Hypothèse & Méthode de Calcul Eurocode 2']],
      descenteRows,
      {
    ...
    const semelleRows = [
      ...
      ["Enrobage Réglementaire des Aciers", enrobageAciers, "Obligation Eurocode 2 pour prévenir la corrosion des armatures"],
    ...
    const etapesTeledac = [
      ...
      ["3. Note de Calcul de Stabilité", "Bureau d'Études Techniques (BET)", "Justification des sections de béton et armatures selon Eurocode 2."],
      ...
      ["5. Contrat & Clauses Code Civil", "Entreprise Générale / Tâcheron", "Imposer le contrat type avec retenue de garantie 5% et respect des 6 points d'arrêt."]
    ];
  ...
  function renderExpress(doc, data, refDoc, currentDate) {
    ...
    // =========================================================================
    drawUnifiedHeader(doc, "Bordereau Quantitatif Estimatif (BQE) Gros œuvre", "Partie I : Métré Volumique Béton & Besoins en Matériaux Structurels (Eurocode 2)", refDoc, currentDate, clientName, clientPhone, lotNumber, 'express');
    ...
    let currentY = 90;
    drawSectionTitle(doc, currentY, "I. SYNTHÈSE DES RATIOS D'INGÉNIERIE & CUBATURES PRINCIPALES (Eurocode 2)");
    ...
    const securiteRows = [
      ...
      ["Calage d'Enrobage Préconisé", `${enrobageCmStr} cm avec cales béton`, isMarine ? "Milieu marin agressif (Eurocode 2, art. A.7.2.4)" : "Milieu non agressif standard (recommandation Eurocode 2)"],
    ...
    currentY = 52;
    drawSectionTitle(doc, currentY, "VII. RÉCAPITULATIF BUDGÉTAIRE GROS œUVRE & CLÉS DE PAIEMENT CONTRAT Code Civil");
    ...
    currentY = doc.lastAutoTable.finalY + TITLE_BEFORE_GAP_MM;
    drawSectionTitle(doc, currentY, "VIII. DÉLAIS DE DÉCOFFRAGE RECOMMANDÉS (RÈGLES PROFESSIONNELLES Eurocode 2)");
    ...
    const refSources = [
      ...
      "• Eurocode 2 (règles professionnelles, référence technique) ;",
...
function renderAudit(doc, data, refDoc, currentDate) {
  ...
  function renderFinitions(doc, data, refDoc, currentDate) {
    ...
    currentY = doc.lastAutoTable.finalY + TITLE_BEFORE_GAP_MM;
    drawSectionTitle(doc, currentY, "VIII. PROCÈS-VERBAL DE RÉCEPTION CONTRADICTOIRE DES TRAVAUX (Code Civil)");
    ...
    doc.setFontSize(7.8);
    doc.setTextColor(...COLOR_NAVY);
    doc.text("ACTE JURIDIQUE DE RÉCEPTION DES TRAVAUX (ARTICLE 740 DU Code Civil)", MARGIN_LEFT + 4, pvBoxY + 5.5);
    ...
    doc.text("La retenue de garantie contractuelle de 5% (Code Civil) demeure consignée jusqu'au PV de levée des réserves.", MARGIN_LEFT + 4, pvBoxY + 43);
  ...
  // =========================================================================
  // EXPORTATION GLOBALE & GESTIONNAIRE DE T�L�CHARGEMENT
  // =========================================================================

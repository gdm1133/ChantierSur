// --- MODULE AUDIT V3.2 : OCR & VALIDATION REEL ---
...
window.auditOcrMetadata = { pages: 0, totalExtrait: 0, totalDoc: 0, entreprise: '', date: '', devisNum: '' };
...
window.processExtractedText = function(text, numPages) {
  ...
  let firstValidLine = "";
  let extractedDate = "";
  let extractedDevisNum = "";
  
  lines.forEach((lineText, idx) => {
    lineText = lineText.trim();
    if (!lineText) return;
    
    const lowLine = lineText.toLowerCase();
    
    // EXCLUDE HEADERS AND FOOTERS FROM LINES
    if (lowLine.includes('page ') || lowLine.includes('fcfa') && lowLine.length < 15) return;
    if (lowLine.includes('ninea') || lowLine.includes('rccm') || lowLine.includes('tva')) return;
    if (/^devis\s*(n°|num)?\s*[\w\d-]+$/i.test(lowLine)) return;
    
    // Metadata parsing
    if (!firstValidLine && lowLine.length > 3 && !lowLine.includes('devis') && !lowLine.includes('date')) {
      firstValidLine = lineText; // Usually company name
    }
    const dateMatch = lineText.match(/\b\d{2}[\/\.-]\d{2}[\/\.-]\d{4}\b/);
    if (dateMatch && !extractedDate) extractedDate = dateMatch[0];
    
    const devisMatch = lineText.match(/(?:devis|réf|facture)\s*(?:n°|num|:)?\s*([a-zA-Z0-9-]+)/i);
    if (devisMatch && !extractedDevisNum) extractedDevisNum = devisMatch[1];
    
    // Lot detection
    if (lineText.length < 50 && (lowLine.includes('lot') || lowLine.includes('chapitre') || lineText.toUpperCase() === lineText)) {
      if (lineText.length > 3 && !lowLine.includes('total') && !lowLine.includes('tva')) {
        currentLot = lineText; 
        return; 
      }
    }
    
    // Generic line detection (Designation + Numbers)
    // Looking for lines with at least a price (one big number at the end)
    ...
    if (matches.length >= 1) {
      let total = parseFloat(matches[matches.length - 1]);
      let pu = 0;
      let q = 1;
      let validLine = false;
      let conf = 'low';
      ...
      if (matches.length >= 2) {
        pu = parseFloat(matches[matches.length - 2]);
        q = matches.length >= 3 ? parseFloat(matches[matches.length - 3]) : 1;
        
        // Strict structure check: q * pu ≈ total
        if (Math.abs((q * pu) - total) < (total * 0.15)) {
          validLine = true;
          conf = 'high';
        } else {
          // Permissive: maybe total is just pu and q is 1
          if (pu > 0 && Math.abs(pu - total) < (total * 0.15)) {
             q = 1;
             validLine = true;
          }
        }
      } else if (total > 1000) {
        // Only one number but it's large, might be a forfait
        pu = total;
        q = 1;
        validLine = true;
      }
      
      if (validLine && !lowLine.includes('total') && !lowLine.includes('net à payer')) {
        let des = lineText;
        matches.forEach(m => des = des.replace(m, ''));
        des = des.replace(/fcfa|f cfa/gi, '').replace(/m2|m3|m²|m³|ml|u|ens|forfait/gi, '').replace(/[^a-zA-ZÀ-ÿ\s-]/g, '').trim(); 
  ...
  // Total doc heuristic
  const totalDocMatch = text.match(/(?:total|net\s*à\s*payer|montant)[\s\S]{0,100}?([\d\s,\.]+)/i);
  ...
  if (extractedLines.length === 0) {
    showError("Aucune ligne détectée. Veuillez réessayer avec un document plus net ou sans ratures.");
    return;
  }

  window.auditOcrLines = extractedLines;
  window.auditOcrMetadata = { 
    pages: numPages, 
    totalExtrait: totalExtracted, 
    totalDoc: totalDoc,
    entreprise: firstValidLine.substring(0, 50),
    date: extractedDate,
    devisNum: extractedDevisNum
  };
  ...
  if (recEl) {
    ...
    recEl.innerHTML = `
      <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
        <div class="text-sm text-blue-900 w-full md:w-1/3">
        ...
        <div class="text-sm text-blue-900 w-full md:w-1/3 border-l border-blue-200 pl-4">
          <p class="font-bold">Métadonnées détectées</p>
          <p class="truncate" title="${window.auditOcrMetadata.entreprise}">Entr.: ${window.auditOcrMetadata.entreprise || 'Non détectée'}</p>
          <p>N° ${window.auditOcrMetadata.devisNum || '-'} • Date: ${window.auditOcrMetadata.date || '-'}</p>
        </div>
        <div class="text-sm text-right w-full md:w-1/3 border-l border-blue-200 pl-4">
...
window.finishOCRValidation = function() {
  ...
  // Populate metadata to the form
  const inputs = document.getElementById('audit-manual-form').elements;
  if(inputs['company_name'] && window.auditOcrMetadata.entreprise) inputs['company_name'].value = window.auditOcrMetadata.entreprise;
  if(inputs['devis_number'] && window.auditOcrMetadata.devisNum) inputs['devis_number'].value = window.auditOcrMetadata.devisNum;
